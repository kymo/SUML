...
int main(int argv, char* argc[]) {
	...
	if (argv < 5) {
        std::cout << "Usage: ./test_tree [training_file] [tree_cnt] [max_depth] [max_node_path] [multi thread or not]" << std::endl;
        return 0;
    }
	...
	suml::rf::RandomForestRegressor *rf = new suml::rf::RandomForestRegressor(tree_cnt, maxNodePath, maxDepth, min_sample_cnt, isMultiThreadOn);
	...
	for (size_t i = 0; i < vctTrainFeature.size(); ++i) {
        std::cout << i << " " << vctTrainLabel[i] << " vs " << rf->predict(vctTrainFeature[i]) << std::endl;
	...
	for (size_t i = 0; i < vctTestFeature.size(); ++i) {
        std::cout << vctTestLabel[i] << " vs " << rf->predict(vctTestFeature[i]) << std::endl;
